...
| Version | Date | Type | What changed |
|---|---|---|---|
| 0.9.2 | 2026-09-12 | patch | **feature 37 skill re-bundle (F25–F26 fold + P4)**: publishes the skills bundled since 0.9.1 — the phase-lint paste fence (`execute-phase` 4.5.2, `plan-fix` 3.2.2, `plan-feature-scaffold` 2.3.2), the P4 deterministic-linter hand-off wording (`execute-phase` 4.5.0, `plan-fix` 3.2.0, `plan-feature-scaffold` 2.3.0), and `phase-contract` 1.0.1 → 1.0.3 (the rule-1 `Hardening & PR` authorization and the rule-3 **≥ 1-task minimum**: a zero-task phase now BLOCKs box-3 instead of passing all eight boxes vacuously), all of which were mirrored into the package but never published. No package code change — the bump exists to publish the re-bundle. |
