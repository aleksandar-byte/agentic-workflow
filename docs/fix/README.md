...
| [#221](https://github.com/gtrabanco/agentic-workflow/issues/221) | sensor-null-parent-receipt | `done · [#223](https://github.com/gtrabanco/agentic-workflow/pull/223)` | `workflow-status` senses every in-progress fix unit's plan receipt as the false `label: missing`: `Parent SPEC snapshot: null` parses as the string `"null"`, the sensor spawns `verify --parent null`, and the verifier refuses — so the one lane that must not send an operator to `/review-plan` on a passing unit does. Fix: null-word lineage lines parse as absent and fix units never pass `--parent` (they carry no spec receipt); rides PR #212 as riding content (ED12). |
